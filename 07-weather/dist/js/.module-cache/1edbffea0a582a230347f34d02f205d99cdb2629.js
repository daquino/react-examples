...
var WeatherApp = React.createClass({displayName: "WeatherApp",
  retrieveForecast: function(location) {
    ...
    }).done(this.processForecastResponse).fail(this.processForecastFailure)
  },
  ...
  processForecastError: function(xhr, exception) {
    this.setState({
      forecast: [],
      selectedIndex: -1,
      forecastFound: false,
...
var LocationForm = React.createClass({displayName: "LocationForm",
  ...
  buildLocationFeedback: function() {
    ...
    if(this.props.forecastAttempted) {
      ...
      locationFeedback = React.createElement("span", {className: "glyphicon glyphicon-refresh glyphicon-refresh-animate form-control-feedback", "aria-hidden": "true"})
    }
